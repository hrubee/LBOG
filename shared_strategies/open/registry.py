...
for _p in (
    ...
    os.path.join(_THIS_DIR, "lbog"),  # lbog module
...
def range_scalper_strategy(df: pd.DataFrame, bb_period: int = 14, bb_std: float = 1.5, bw_threshold: float = 0.008, vol_ratio: float = 0.8, rsi_period: int = 7, rsi_ob: float = 70, rsi_os: float = 30) -> pd.DataFrame:
    return range_scalper_core(df, bb_period=bb_period, bb_std=bb_std, bw_threshold=bw_threshold, vol_ratio=vol_ratio, rsi_period=rsi_period, rsi_ob=rsi_ob, rsi_os=rsi_os)
